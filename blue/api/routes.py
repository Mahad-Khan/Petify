...
class Lost(Resource):
    def post(self):
        try:
            ...
            exists = bool(Pet.query.filter_by(id=dog_id).first())
            print(exists)
            if exists:
                ...
                exists = bool(Found.query.filter_by(breed=lost_dog_breed).first())#check this breed exists or not
                print(exists)
                if exists:
                    found_db =Found.query.filter(Found.breed == lost_dog_breed).all()
                    print(found_db)
...
class Yes_Found(Resource):
    def post(self):
        try:
            ...
            if pet_url =='' or pet_url==None: ##tables will update using guest id
                exists=bool(Found.query.filter_by(gid=guest_id).first())
                if not(exists):
                ...
                return jsonify(ret)
            else:                              ##tables will update using url
                exists=bool(Found.query.filter_by(url=pet_url).first())
                if not(exists):
                    ret={"status":200,"msg":"url not found"}
                    return jsonify(ret)
                db.session.query(Found).filter(Found.url==pet_url).delete()
                db.session.commit()
                pet_row=Pet.query.filter_by(id=pet_id).first()
                pet_row.lost='f'
                db.session.commit()
